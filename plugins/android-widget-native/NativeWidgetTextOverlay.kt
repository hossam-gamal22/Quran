...
object NativeWidgetTextOverlay {
  ...
  private const val IMAGE_AUTHORITY_SUFFIX = ".rnwidget.imageprovider"
  ...
  fun refresh(context: Context, manager: AppWidgetManager, widgetIds: IntArray) {
    for (widgetId in widgetIds) {
      try {
        val overlays = storedOverlays(context, widgetId) ?: continue
  ...
  fun restoreCachedSnapshots(context: Context, manager: AppWidgetManager, widgetIds: IntArray) {
    for (widgetId in widgetIds) {
      try {
        val lightFileName = imageFileName(widgetId, "light")
        val darkFileName = imageFileName(widgetId, "dark")
        val imageFolder = context.filesDir.resolve("widget_images")
        if (!imageFolder.resolve(lightFileName).exists()) continue
        val resolvedDarkFileName = if (imageFolder.resolve(darkFileName).exists()) darkFileName else lightFileName
        val views = RemoteViews(context.packageName, R.layout.rn_widget)
        views.setImageViewUri(R.id.rn_widget_image_light, imageUri(context, lightFileName))
        views.setImageViewUri(R.id.rn_widget_image_dark, imageUri(context, resolvedDarkFileName))
        hideAll(views)
        val overlays = storedOverlays(context, widgetId)
        if (overlays != null) {
          for (index in 0 until minOf(overlays.length(), TEXT_IDS.size)) {
            configure(context, views, index, overlays.getJSONObject(index))
          }
        ...
        Log.i(TAG, "Restored cached snapshot widgetId=$widgetId")
      } catch (error: Exception) {
        Log.e(TAG, "Failed cached snapshot restore widgetId=$widgetId", error)
      }
    }
  }

  private fun storedOverlays(context: Context, widgetId: Int): JSONArray? {
    val raw = context
  ...
  private fun imageFileName(widgetId: Int, mode: String): String {
    return "widget_${widgetId}_mode_$mode.png"
  }

  private fun imageUri(context: Context, fileName: String): Uri {
    return Uri.Builder()
      .scheme("content")
      .authority(context.packageName + IMAGE_AUTHORITY_SUFFIX)
