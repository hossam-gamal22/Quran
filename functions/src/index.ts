...
import { normalizeWeights, computeScore } from './score-weights';
...
function buildGenericSearchLinks(question: string, language = 'ar'): AutoQaSource[] {
  ...
  const siteSearch = (site: string) =>
    `https://www.google.com/search?q=${encodeURIComponent(`site:${site} ${question}`)}`;
  if (!isArabicLanguage(language)) {
    return [
      {
...
export const selectMonthlyWinners = onSchedule(
  { schedule: '0 12 1 * *', timeZone: 'Africa/Cairo', secrets: ['EXPO_ACCESS_TOKEN'] },
  async () => {
    try {
      const now = new Date();
      ...
      type Candidate = { userId: string; displayName: string; score: number; data: FirebaseFirestore.DocumentData };
 ...
 */
export const cacheLeaderboardSnapshot = onSchedule(
  ...
  async () => {
    try {
      ...
      const entries: Array<{ userId: string; displayName: string; score: number }> = [];
 ...
 */
async function rebuildLeaderboardCache(currentMonth: string): Promise<void> {
  const snapshot = await db.collection('users')
    .where('monthlyEngagement.month', '==', currentMonth)
    .orderBy('monthlyEngagement.score', 'desc')
    .limit(50)
 ...
 */
export const recomputeScoresOnWeightsChange = functions
  ...
  .onUpdate(async (change) => {
    ...
    if (afterVersion <= beforeVersion) return; // only react to a weights bump

    const weights = normalizeWeights(after.scoreWeights || {});
    const now = new Date();
    const currentMonth = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-v2`;
    ...
    for (const doc of snap.docs) {
      ...
      const newScore = computeScore(me.activities || {}, weights);
      if (newScore === oldScore && Number(me.weightsVersion) === afterVersion) continue;
 ...
 * traffic window.
 */
export const cleanupActivityDaily = onSchedule(
  { schedule: '0 2 * * *', timeZone: 'Asia/Riyadh' },
  async () => {
