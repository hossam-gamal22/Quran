        ...
        <TouchableOpacity activeOpacity={1}>
          <View style={{ backgroundColor: 'rgba(120,120,128,0.12)', borderRadius: 20, padding: 24, width: 280, alignItems: 'center' }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 24 }}>
              <View>
                <Text style={{ fontSize: 12, color: colors.muted, textAlign: 'center', marginBottom: 4 }}>الساعة</Text>
                <TextInput
              ...
              <View>
                <Text style={{ fontSize: 12, color: colors.muted, textAlign: 'center', marginBottom: 4 }}>الدقيقة</Text>
                <TextInput
                  style={{ backgroundColor: colors.background, borderWidth: 2, borderColor: accentColor, borderRadius: 12, width: 70, height: 50, textAlign: 'center', fontSize: 22, fontWeight: '800', color: colors.foreground }}
                  value={minute}
...
export default function NotificationsCenterScreen() {
  ...
  const handleRequestPermission = async () => {
    ...
    if (!granted) {
      Alert.alert(
        'إذن الإشعارات',
        'يرجى تفعيل إشعارات التطبيق من إعدادات الهاتف لتلقي التذكيرات',
        [{ text: 'حسناً' }]
      );
    }
  };
  ...
  const s = StyleSheet.create({
    ...
    rowSub: { fontSize: 12, color: colors.muted, textAlign: 'right' },
    ...
    // Prayer row
    prayerRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 11 },
    prayerEmoji: { fontSize: 18, marginLeft: 10 },
  ...
  const PRAYERS = [
    { key: 'Fajr',    emoji: '🌙', name: 'الفجر' },
    { key: 'Dhuhr',   emoji: '☀️', name: 'الظهر' },
    { key: 'Asr',     emoji: '🌤', name: 'العصر' },
    { key: 'Maghrib', emoji: '🌇', name: 'المغرب' },
    { key: 'Isha',    emoji: '✨', name: 'العشاء' },
  ] as const;
  ...
  return (
    <ScreenContainer containerClassName="bg-background" edges={['top', 'left', 'right', 'bottom']}>
      <View style={s.header}>
        <Text style={s.title}>🔔 مركز الإشعارات</Text>
        <Text style={s.subtitle}>تحكم في جميع التذكيرات</Text>
      </View>

      {saving && (
        <View style={s.savingBadge}>
          <Text style={{ fontSize: 11, fontWeight: '700', color: colors.success }}>✓ تم الحفظ</Text>
        </View>
      )}
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 50 }}>
        ...
        {!hasPermission && (
          <TouchableOpacity style={s.permBanner} onPress={handleRequestPermission}>
            ...
            <Text style={{ flex: 1, fontSize: 13, color: colors.warning, fontWeight: '600', textAlign: 'right' }}>
              يلزم تفعيل إذن الإشعارات لتلقي التذكيرات — اضغط هنا
            </Text>
          </TouchableOpacity>
        )}
          ...
          {PRAYERS.map((p, idx) => (
            <View key={p.key}>
              ...
              <View style={s.prayerRow}>
                ...
                <Text style={s.prayerEmoji}>{p.emoji}</Text>
              </View>
            </View>
          ))}
        ...
        <View style={[s.card, !settings.wirdEnabled && { opacity: 0.5 }]}>
          <View style={s.row}>
            <View style={s.rowRight}>
              <TouchableOpacity
                style={s.timeBtn}
                onPress={() => setTimePicker({ key: 'wirdMorningTime', value: settings.wirdMorningTime, title: '🌅 وقت الورد الصباحي' })}
            ...
            <View style={{ flex: 1 }}>
              <Text style={s.rowLabel}>الورد الصباحي</Text>
              <Text style={s.rowSub}>أذكار الصباح</Text>
            </View>
          ...
          <View style={s.row}>
            <View style={s.rowRight}>
              <TouchableOpacity
                style={s.timeBtn}
                onPress={() => setTimePicker({ key: 'wirdEveningTime', value: settings.wirdEveningTime, title: '🌇 وقت الورد المسائي' })}
                disabled={!settings.wirdEnabled}
              >
            ...
            <View style={{ flex: 1 }}>
              <Text style={s.rowLabel}>الورد المسائي</Text>
              <Text style={s.rowSub}>أذكار المساء</Text>
            </View>
            <Text style={{ fontSize: 20, marginLeft: 8 }}>🌇</Text>
          </View>
        </View>
          ...
          <View style={s.row}>
            <View style={s.rowRight}>
              <TouchableOpacity
                ...
                onPress={() => setTimePicker({ key: 'kahfTime', value: settings.kahfTime, title: '📖 وقت تذكير الكهف (الجمعة)' })}
            ...
            <View style={{ flex: 1 }}>
              <Text style={s.rowLabel}>تذكير يوم الجمعة</Text>
              <Text style={s.rowSub}>يصلك كل جمعة بإذن الله</Text>
            </View>
            <Text style={{ fontSize: 20, marginLeft: 8 }}>📅</Text>
          </View>
        </View>
          ...
          <View style={s.row}>
            <View style={s.rowRight}>
              <TouchableOpacity
                ...
                onPress={() => setTimePicker({ key: 'dailyAyahTime', value: settings.dailyAyahTime, title: '✨ وقت آية اليوم' })}
            ...
            <View style={{ flex: 1 }}>
              <Text style={s.rowLabel}>آية كريمة يومياً</Text>
              <Text style={s.rowSub}>تذكير يومي بكلام الله</Text>
            </View>
            <Text style={{ fontSize: 20, marginLeft: 8 }}>🌟</Text>
          </View>
        </View>
        ...
        <View style={{ marginHorizontal: 12, marginTop: 20, padding: 16, backgroundColor: 'rgba(120,120,128,0.12)', borderRadius: 16, borderWidth: 1, borderColor: colors.border }}>
          ...
          {[
            { on: settings.adhanEnabled, label: 'أذان الصلاة', emoji: '🕌' },
            { on: settings.wirdEnabled, label: 'الورد اليومي (مرتين)', emoji: '📿' },
            { on: settings.kahfEnabled, label: 'سورة الكهف (جمعة)', emoji: '📖' },
            { on: settings.dailyAyahEnabled, label: 'آية اليوم', emoji: '✨' },
          ].map((item, i) => (
            <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: item.on ? colors.success : colors.border }} />
              <Text style={{ flex: 1, fontSize: 13, color: item.on ? colors.foreground : colors.muted, textAlign: 'right' }}>
